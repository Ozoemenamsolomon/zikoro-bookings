...
const AppointmentHistory = ({bookingsData,countSize,errorString,initialItem}:{
    ...
    const fetchAppointments = async (page: number = 1) => {
      ...
      try {
        ...
        const response = await fetch(`/api/bookingsContact/fetchBookings?createdBy=${user?.id}&contactEmail=${contact?.email}&offset=${offset}&limit=${limit}&workspaceId=${workspaceId}`)
    ...
    <section className='md:p-3 '>
        <div className="md:border md:rounded-lg bg-white flex-col h-full justify-between">
            <div className="">
                ...
                <section className="flex flex-col gap-3 w-full min-h-screen justify-between p-3">
                    {loading ? (
                        <div className="flex gap-3 justify-center w-full h-full text-basePrimary/50 py-20"><Loader2Icon className='animate-spin'/></div>
                    ) : isError ? (
